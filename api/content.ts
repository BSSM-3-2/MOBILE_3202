...
export const createPost = async (payload: CreatePostPayload): Promise<Post> => {
    ...
    payload.images?.forEach((img, idx) => {
        console.log(`[createPost] Appending image ${idx}:`, {
            uri: img.uri,
            name: img.name,
            type: img.type,
        });
    ...
    console.log('[createPost] Starting upload with apiClient');

    try {
        const res = await apiClient.post<Post>('/content', form, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
        console.log('[createPost] Upload successful');
        return res.data;
    } catch (error) {
